...
# Debug: Check whether API keys are loaded from .env
st.write("Gemini Key:", bool(os.getenv("GOOGLE_API_KEY")))
st.write("Groq Key:", bool(os.getenv("GROQ_API_KEY")))
st.write("OpenAI Key:", bool(os.getenv("OPENAI_API_KEY")))
st.write("Gemini Model:", os.getenv("GEMINI_MODEL"))
st.write("Groq Model:", os.getenv("GROQ_MODEL"))
st.write("OpenAI Model:", os.getenv("OPENAI_MODEL"))

st.set_page_config(page_title="Gemini Agentic AI - Trip Monitoring", page_icon="🤖", layout="wide")
st.title("🤖 Multi-Model Agentic AI Chat: Trip Monitoring Assistant")
st.write("Upload trip data and ask questions. Gemini/OpenAI/Groq + LangChain agent will select the correct tool and analyze the data.")
...
st.sidebar.write("Supported `.env` keys:")
...
st.sidebar.info("Fallback order: Gemini → OpenAI → Groq. Raw Gemini metadata/signature is cleaned.")
...
if uploaded_file is not None:
    ...
    if user_question:
        ...
        with st.chat_message("assistant"):
            with st.spinner("Agent is selecting the best available model and correct tool..."):
...
st.markdown("---")
st.caption("Multi-Model Agentic AI Project | Gemini/OpenAI/Groq + LangChain + Streamlit + Pandas")
